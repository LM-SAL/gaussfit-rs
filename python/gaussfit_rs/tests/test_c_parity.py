...
``make_c_reference.py``; ``test_matches_live_c_extension`` also runs whenever ``muse`` is
importable. See ``helpers.assert_fit_parity`` for the contract, checked family by family so a
failure names the kind of spectrum.
"""
...
def test_matches_live_c_extension(name):
    pytest.importorskip("muse.fastfit.fit_single_spectrum_ext")
